...
function BattleRender.draw(graphics, model, layout, font, theme)
  ...
  if model.enemy and model.enemy.sprite then
    ...
    if model.animation and model.animation.kind == "intro" then
      local progress = model.animation.progress or 0
      spriteRect = {
        x=layout.enemySprite.x + (1 - progress) * layout.field.w * 0.35,
        y=layout.enemySprite.y, w=layout.enemySprite.w, h=layout.enemySprite.h,
      }
    end
    spriteRect = animatedSpriteRect(spriteRect, "enemy")
    if spriteRect then
  ...
  if model.player and model.player.sprite then
    ...
    if model.animation and model.animation.kind == "intro" then
      local progress = model.animation.progress or 0
      spriteRect = {
        x=layout.playerSprite.x - (1 - progress) * layout.field.w * 0.35,
        y=layout.playerSprite.y, w=layout.playerSprite.w, h=layout.playerSprite.h,
      }
    end
    spriteRect = animatedSpriteRect(spriteRect, "player")
    if spriteRect then
